...
Implements Monte Carlo inference for Gaussian hierarchical linear models of the following form:
    Y[i·]|(X, Bet, tau2) ~ Mv-Normal(Bet[i·] * X, tau2 * I) [emission distribution]
...
Alternatively, the model may be expressed in its non-centered parameterization:
    Y[i·]|(X, Bet, tau2) ~ Mv-Normal((Gam[i·] + mu) * X, tau2 * I) [emission distribution]
...
from semi_conjugate import mlm_mnorm as mrmn
...
def update_block1(Y: np.ndarray,
    ...
    return mrmn.update(*reshape(Y, X, mu, Sig, tau2))
...
def update_block2_ncp(Y: np.ndarray,
    ...
    emitN = rmnc.update(emit_train.T, np.tile(X, Y.shape[0]), np.zeros([1, X.shape[0]]), np.zeros(2 * [X.shape[0]]), *emit0)
...
def estimate_block1(Y: np.ndarray,
    ...
    Bet, = mrmn.get_ev(*update_block1(Y, X, mu, Sig, tau2))
    Gam = Bet - mu
...
def sample_block1(Y: np.ndarray,
    ...
    Bet, = mrmn.sample_param(1, *update_block1(Y, X, mu, Sig, tau2))[0]
    Gam = Bet - mu
    rho = eval_rate_convergence(X, Sig, tau2)
...
def eval_logobserved(Y: np.ndarray, X: np.ndarray, mu: np.ndarray, Sig: np.ndarray, tau2: float) -> float:
    ...
    return mrmn.eval_logmargin(*reshape(Y, X, mu, Sig, tau2))
